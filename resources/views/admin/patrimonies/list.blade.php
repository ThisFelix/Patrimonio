                            ...
                            <table class="table table-hover table-bordered display" id="patrimonies">
                                ...
                                @foreach($patrimonies as $patrimony)
                                    <tr>
                                        <th scope="row">
                                        {{ $patrimony->id }}</td>
                                        <td>@if($patrimony->status == 1) Disponivel @elseif($patrimony->status == 2)
                                                Indisponivel @endif</td>
                                        ...
                                        <td>
                                            @if($patrimony->image != null && $patrimony->image != '')
                                                <img src="{{ url('storage/patrimony/'.$patrimony->image) }}"
                                                     alt="{{ $patrimony->name }}" class="img-responsive"
                                        ...
                                        <td>
                                            <a class="btn btn-default"
                                               href="{{ route('patrimonies.edition', $patrimony->id) }}">Editar</a>
                                            <a class="btn btn-danger"
                                               href="javascript:(confirm('Deletar registro?') ? window.location.href = '{{ route('patrimonies.delete', $patrimony->id)}}' : false)">Deletar</a>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
                            ...
                            <div class="col-sm-12 my-4">
                                    ...
                                    <div class="col-sm-12">
                                        ...
                                        <select class="form-control" name="location" required="">
                                            ...
                                            @foreach($rooms as $room)
                                                <option value="{{ $room->id }}">Prédio: {{ $room->building_name->name }}
                                                    - Sala: {{ $room->number }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="row" style="padding-top: 10px;">
                                    <div class="col-sm-6">
                                        ...
                                        <input type="number" class="form-control" name="init_number" placeholder="0"
                                               min="0" required="">
                                    </div>
                                    <div class="col-sm-6">
                                        <label for="nome">Número final</label>
                                        <input type="number" class="form-control" name="final_number" placeholder="0"
                                               min="0" required="">
                                    </div>
                                </div>
                            </div>
